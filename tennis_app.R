...
df <- bind_rows(w_small, l_small) %>%
  rename(player = "value")
df
df_1 <- df %>% filter(player == "Daniil Medvedev")
ggplot(data = df_1, aes(x = ace)) +
  geom_histogram()

library(shiny)

ui <- fluidPage(
  sidebarPanel(selectizeInput(inputId = "player",
...
server <- function(input, output, session) {
  ...
  output$playerhist <- renderPlot({
    ggplot(data = df_oneplayer(), aes_string(x = input$variable)) +
